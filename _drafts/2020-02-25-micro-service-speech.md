...
我在整理的过程中，就发现，原来之前我接触的那些工作，实在是冰山一角，微服务涉及到的内容，实在太多了，就像之前同事 JS 分享过的「数据挖掘和人工智能」课题一样，听上去很简单的一个词，其实里面的内容很深且广。
...
- 第二点，讲一下微服务架构到底包含哪些内容，或者说包含哪些技术；
- 第三点，接着上周分享的产品架构，简单谈一下咱们的产品，接下来可以尝试从哪个方向去做，当然这里只是给个建议，具体技术实现，可能还需要技术团队去研究。
...
最开始的时候，网站的功能很简单，实现一些核心的功能，满足用户基本需求就可以了，所以，第一版产品很快就上线了，它的架构大概如下图所示：

![image-20200302114756010](C:\Users\decad\Git\blog-bitcron\_drafts\image-20200302114756010.png)

这大概是很多初创团队产品最初的架构，分为前台，后台，数据库。

然而随着业务的发展，出现了各种竞争对手，在竞争的压力下，小皮认为，需要开展一些营销手段了。

除了新增营销的相关功能，为了精准营销，还开发了数据分析模块，同时，也支持了移动端设备，开发了微信小程序以及 APP。

开发任务比较重，于是团队又加了一个人，负责移动端的开发。在这个阶段，产品的架构图大致如下图所示：

![image-20200302120441638](C:\Users\decad\Git\blog-bitcron\_drafts\image-20200302120441638.png)
...
- 管理后台在加入数据分析和促销管理相关功能后出现性能瓶颈，影响了其他应用；
- 开发、测试、部署、维护愈发困难。即使只改动一个小功能，也需要整个应用一起发布；
- 团队出现推诿扯皮现象。关于一些公用的功能应该建设在哪个应用上的问题常常要争论很久，最后要么干脆各做各的，或者随便放个地方但是都不维护；

团队很快发现，这不是长久之计，需要对产品架构进行改造，小明和小红，在完成正常业务的开发外，牺牲了大部分的业余时间，对一些通用的功能模块进行抽象封装，最后产品架构图如下：

![image-20200302120540085](C:\Users\decad\Git\blog-bitcron\_drafts\image-20200302120540085.png)

在这个阶段，已经有了服务化的概念，将通用的一些功能模块形成公用服务，供前台调用。

虽然业务服务分开了，但是依然有一些问题，例如：数据库依然公用，对数据库的调用依然耦合在一起。

因此，团队在一鼓作气，将数据库也拆开了，各个业务模块形成完全独立的服务。产品架构图如下：

![image-20200302120627092](C:\Users\decad\Git\blog-bitcron\_drafts\image-20200302120627092.png)

到这里就有了微服务的雏形，这个架构看似非常的完善，但是，面临越来越多的微服务时，它将会暴露出越来越多的问题，而**微服务架构**提出了 6 大组件，就是为了解决这些问题，后面再详细谈到。

现在简单回顾一下，微服务产生的几个发展阶段：

![](C:\Users\decad\Git\blog-bitcron\_drafts\2020-02-28-21-01-13.png)

第一阶段：单体应用阶段，最常见的就是 LAMP 架构，对应上面故事中，前两个发展阶段。这一阶段的优点是学习成本地，开发快，可以快速对产品进行试错，但是缺点也很明显。

第二阶段：服务化，抽象出公用的业务逻辑，形成公共服务。从本地方法调用的方式，变成通过接口远程调用。这个阶段，虽然采用了服务化的思路，但是，某种程度来讲，并不彻底。

第三阶段，微服务，随着 Docker 为代表的容器化技术以及敏捷开发测试的文化驱动下，使得服务化思想进一步发展，形成微服务。它与服务化的差别就是：拆分粒度更细；服务独立部署，独立维护。

而随之带来的问题就是，对服务治理能力要求高。

### 微服务架构

一种技术在解决一个问题的同时，势必会带来另外一个问题。我们是否要采取某个技术方案，也是看，带来的利是否大于弊。

上面我们已经介绍了微服务的发展，可以看出，他解决了很多的发展过程中的问题，但是，也出现了很多之前单体应用不会出现的问题。

例如：服务如何定义？服务如何发布和订阅？服务如何监控？服务故障如何定位？服务如何治理？

这些问题，会随着微服务数量的增多，变得越来越突出。以前单体应用可能压根就不会涉及到，而微服务就必须要去解决这些问题。

所以，微服务架构就发展出了 6 大组件，分别为：服务描述、注册中心、服务框架、服务监控、服务追踪以及服务治理。

它们就是解决微服务发展过程中的那些问题而诞生。

![](./_image/2020-02-28-21-00-04.png)

所以，我们在谈到微服务架构的时候，就要考虑到这 6 个组件如何运转，如何部署。下面将分别来说说，这 6 个组件都规范了哪些内容。
...
这种方案的优点是，服务调用方几乎没有学习成本，适用于跨业务平台之间的调用。例如我们常看到平台提供 URL 供开发者调用，采用的就是这种方式，例如： https://api.github.com。
...
借着上面的比喻，服务框架就是两个人如何相识，是面谈还是网聊？面谈的话，是走地铁还是公交？网聊的话，是语音还是视频？

接下来就是微服务的管理成本了。

#### 服务监控

首先是如何知道服务调用是否正常，这个问题可不像单体应用那样，是否正常很容易就能看出来。

随着服务数量的增多，服务与服务之间的通信以及调用状态，需要一种机制快速的查看的，这就是服务监控组件的作用。
...
准确的说，服务治理讲的是，通过一系列的手段来保证在各种意外情况下，服务调用仍然能够正常进行。
...
微服务架构可以说是 ESB 的进一步细化，是它的升级版，ESB 更多是解决不同平台之间的数据通信问题。

而微服务架构进一步细化，将单个应用程序的内部再分解为小块，这些小块可独立更改，扩展和管理，随着虚拟化，云计算，敏捷开发实践和 DevOps 的兴起，微服务应运而生。
